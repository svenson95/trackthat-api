...
import com.svenson95.trackthat_api.database.dto.ExerciseWorkoutHistoryDTO;
import com.svenson95.trackthat_api.database.dto.LogWorkoutDTO;
import com.svenson95.trackthat_api.database.mapper.LogWorkoutMapper;
import com.svenson95.trackthat_api.database.model.LogWorkout;
import com.svenson95.trackthat_api.database.repository.LogWorkoutRepository;
...
public class LogWorkoutService {
  ...
  public ResponseEntity<List<LogWorkoutDTO>> findLogWorkoutsForUser(String userId) {
    List<LogWorkoutDTO> logs = logWorkoutRepository.findAllByUserId(userId).stream().map(logWorkoutMapper::toDto)
        .toList();
  ...
  public ExerciseWorkoutHistoryDTO findWorkoutHistoryForExercise(
    ...
    long beforeExclusive = before != null ? before : Instant.now().minus(WORKOUT_DURATION).getEpochSecond() + 1;

    List<LogWorkout> logs = logWorkoutRepository.findByUserIdAndSetsExerciseAndDateLessThanOrderByDateDesc(
        userId, exercise, beforeExclusive, PageRequest.of(0, normalizedLimit + 1));

    boolean hasMore = logs.size() > normalizedLimit;

    List<LogWorkoutDTO> workouts = logs.stream()
  ...
  private LogWorkoutDTO toExerciseWorkoutDto(LogWorkout log, String exercise) {
    ...
    List<LogWorkoutDTO.SetItemDTO> sets = Optional.ofNullable(log.getSets()).orElseGet(List::of).stream()
  ...
  public LogWorkoutDTO updateOrCreateLog(
    ...
    LogWorkout log = logWorkoutRepository.findAll().stream()
  ...
  public ResponseEntity<LogWorkoutDTO> updateSetsInLog(
    ...
    LogWorkout log = logWorkoutRepository
